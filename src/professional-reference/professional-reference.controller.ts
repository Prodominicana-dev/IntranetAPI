...
import {
  CreateProfessionalReferenceDto,
  UpdateProfessionalReferenceDto,
} from './dto/professional-reference.dto';
import { Response } from 'express';
import { ProfessionalReferenceService } from './professional-reference.service';

@Controller('v1/professional-reference')
export class ProfessionalReferenceController {
  constructor(
    private readonly professionalReferenceService: ProfessionalReferenceService,
  ) {}

  // Crear una referencia profesional
  @Post()
  async create(
    @Body() data: CreateProfessionalReferenceDto,
    @Res() res: Response,
  ) {
    try {
      const professionalReference =
        await this.professionalReferenceService.create(data);
  ...
  async update(
    @Param('id') id: string,
    @Body() data: UpdateProfessionalReferenceDto,
    @Res() res: Response,
  ) {
    try {
      const professionalReference =
        await this.professionalReferenceService.update(id, data);
  ...
  async delete(@Param('id') id: string, @Res() res: Response) {
    try {
      const professionalReference =
        await this.professionalReferenceService.delete(id);
      return res.status(200).json(professionalReference);
    } catch (error) {
      return res.status(400).json({ message: error.message });
    }
  ...
  async getAll(@Param('userId') userId: string, @Res() res: Response) {
    try {
      const professionalReferences =
        await this.professionalReferenceService.getAll(userId);
  ...
  async getById(@Param('id') id: string, @Res() res: Response) {
    try {
      const professionalReference =
        await this.professionalReferenceService.getById(id);
      return res.status(200).json(professionalReference);
    } catch (error) {
      return res.status(400).json({ message: error.message });
    }
