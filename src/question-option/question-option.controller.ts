...
export class QuestionOptionController {
  ...
  async createMany(
    @Param('id') id: string,
    @Body() data: CreateManyQuestionOptionDto[],
    @Res() res: Response,
  ) {
    try {
      ...
      const questionOptions =
        await this.questionOptionService.createMany(newData);
  ...
  async getAll(@Param('questionId') questionId: string, @Res() res: Response) {
    try {
      const questionOptions =
        await this.questionOptionService.getAll(questionId);
